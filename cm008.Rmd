...
# geom_jitter is random. Gives you random patterns each time you run it.
# "lm" = linear regression model
# geom_smooth() fits to the original data points.
```
...
```{r, fig.width = 5, fig.height = 2}
gapminder %>% 
  # group_by(country) %>% --> grouping within ggplot is different from the grouping in dplyr.
  ggplot(aes(year, lifeExp, group=country, color=country=="Rwanda")) +
  geom_line() +
  scale_color_discrete("Countries", labels=c("Other", "Rwanda"))
```
...
# scales="free" would free both x and y axis
# think of ~ as a formula
```
...
- Use `shape=21` (hollow circles) to distinguish between `fill` (interior) and `colour` (exterior). 
...
gapminder %>% 
  ...
  geom_point(aes(size=pop, fill=continent), alpha=0.7, shape=21) +
# `shape()` can take a character as an argument as well.
  scale_x_log10(labels = scales::comma_format()) +
  scale_size_area()
```
